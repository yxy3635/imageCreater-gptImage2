...
<template>
  <div v-if="src" class="fixed inset-0 z-50 grid place-items-center bg-slate-950/78 p-4 backdrop-blur-sm" @click.self="emit('close')">
    <div class="grid max-h-[94vh] w-full max-w-6xl overflow-hidden rounded-[28px] border border-white/20 bg-white p-4 shadow-[0_30px_110px_rgba(2,6,23,0.45)] md:grid-cols-[1.12fr_0.88fr]">
      ...
      <aside class="flex min-h-0 flex-col p-5">
        ...
        <div class="mt-auto flex flex-wrap items-center gap-3 border-t border-slate-100 pt-5">
          <button class="rounded-2xl bg-blue-50 px-4 py-3 text-sm font-black text-blue-600 transition hover:bg-blue-100" @click="downloadImage(src)">下载图片</button>
          <button v-if="record" class="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600 transition hover:bg-red-100" @click="emit('delete', record)">删除任务</button>
          <button class="ml-auto rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-600 transition hover:bg-slate-50" @click="emit('close')">关闭</button>
        </div>
      </aside>
    </div>
