...
<template>
  <AppLayout>
    <section class="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/82 p-6 shadow-[0_30px_100px_rgba(14,165,233,0.12)] backdrop-blur-2xl md:p-8">
      ...
      <div class="relative grid gap-8 lg:grid-cols-[1fr_320px]">
        <div>
          ...
          <h1 class="mt-4 text-4xl font-black tracking-tight md:text-6xl">欢迎回来，{{ auth.userInfo?.username }}</h1>
          <p class="mt-4 max-w-2xl text-sm leading-7 text-slate-600">
            这里汇总你的生成资产、余额状态和最近创作。进入前台创作空间即可继续生成新图像。
          </p>
          <div class="mt-7 flex flex-wrap gap-3">
            <RouterLink class="rounded-full bg-sky-500 px-5 py-3 text-sm font-black text-white shadow-[0_18px_50px_rgba(14,165,233,0.28)] transition hover:-translate-y-0.5 hover:bg-sky-600" to="/create">进入前台创作</RouterLink>
            <RouterLink class="rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-800 transition hover:border-sky-200 hover:bg-sky-50" to="/user/payment">管理余额</RouterLink>
          </div>
        </div>
    ...
    <div class="mt-6 grid gap-4 md:grid-cols-3">
      <div class="soft-card p-5">
        ...
        <p class="mt-2 text-4xl font-black">{{ records.length }}</p>
      </div>
      <BalanceCard :balance="balance" />
      <div class="soft-card p-5">
        <p class="text-sm font-semibold text-slate-500">成功记录</p>
        <p class="mt-2 text-4xl font-black">{{ records.filter((item) => item.status === 'success').length }}</p>
      </div>
    </div>
    ...
    <section class="mt-9">
      <div class="mb-5 flex items-center justify-between">
        ...
        <RouterLink class="btn-secondary rounded-full" to="/user/history">查看全部</RouterLink>
      </div>
