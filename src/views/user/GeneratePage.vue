...
<template>
  <div class="min-h-screen bg-[#fbfbfc] pb-48 text-slate-950">
    <header class="sticky top-0 z-30 border-b border-slate-100 bg-white/86 backdrop-blur-2xl">
      <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-8">
        <div class="flex items-center gap-3">
          <RouterLink class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-slate-700 transition hover:bg-slate-50" to="/user/dashboard">返回控制台</RouterLink>
          <h1 class="text-lg font-black tracking-tight">AI 图像创作台</h1>
        </div>
        <div class="flex items-center gap-2">
          <RouterLink class="rounded-2xl bg-slate-950 px-4 py-2 text-sm font-black text-white" to="/user/history">画廊</RouterLink>
          <RouterLink class="rounded-2xl px-4 py-2 text-sm font-bold text-slate-500 transition hover:bg-slate-100" to="/user/dashboard">资产</RouterLink>
          ...
          <button class="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-black text-slate-700 transition hover:bg-slate-50" @click="logout">退出</button>
        </div>
      </div>
    </header>

    <main class="mx-auto max-w-7xl px-4 py-7 md:px-8">
      <div class="mb-5 flex items-center gap-3">
        ...
        <div class="relative">
          <button
            class="flex h-11 min-w-36 items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700 shadow-sm outline-none transition duration-300 hover:border-sky-200 hover:bg-sky-50/40 focus:border-sky-300 focus:ring-4 focus:ring-sky-100"
            type="button"
      ...
      <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <article
          ...
          :class="record.status === 'pending' ? 'grid-cols-[142px_1fr] border-sky-100 shadow-[0_14px_42px_rgba(14,165,233,0.10)] hover:shadow-[0_20px_55px_rgba(14,165,233,0.12)]' : 'grid-cols-[190px_1fr] border-slate-200 hover:shadow-[0_20px_60px_rgba(15,23,42,0.08)]'"
        >
          <button class="relative overflow-hidden bg-slate-100" @click="previewRecord = record">
            <img v-if="record.generatedImageUrl" :src="record.generatedImageUrl" class="h-full min-h-[188px] w-full object-cover transition duration-500 group-hover:scale-105" alt="生成图像" />
            <div v-else-if="record.status === 'pending'" class="grid h-full min-h-[188px] place-items-center bg-gradient-to-br from-sky-50 to-fuchsia-50">
              <div class="loader loader-compact"></div>
            </div>
            <div v-else class="grid h-full min-h-[188px] place-items-center text-sm font-bold text-slate-400">暂无图像</div>
    ...
    <section class="fixed inset-x-0 bottom-6 z-40 mx-auto w-[min(860px,calc(100%-32px))] rounded-[28px] border border-slate-200 bg-white/94 p-4 shadow-[0_24px_90px_rgba(15,23,42,0.16)] backdrop-blur-2xl">
      <div class="flex items-center gap-3">
        <input
          v-model="prompt"
        ...
        />
        <button class="grid h-12 w-12 place-items-center rounded-2xl border border-slate-200 bg-slate-100 text-slate-500 transition hover:bg-slate-200" title="附件">
        ...
        <button class="grid h-12 w-12 place-items-center rounded-2xl bg-blue-600 text-white shadow-[0_14px_34px_rgba(37,99,235,0.26)] transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60" :disabled="loading || configs.length === 0" @click="generate">
      ...
      <div class="mt-3 grid gap-3 text-xs font-bold text-slate-500 sm:grid-cols-[2fr_0.8fr_0.8fr_1fr]">
        <div class="relative">
      ...
      <div class="mt-3 flex flex-wrap gap-2">
        <button v-for="preset in promptPresets" :key="preset" class="rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-sky-50 hover:text-sky-700" @click="usePreset(preset)">
          {{ preset }}
        </button>
      </div>
    </section>
